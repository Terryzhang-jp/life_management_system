...
class TasksDatabaseManager {
  ...
  // Get all schedulable tasks (level 2 tasks and level 1 tasks without children)
  getAllSchedulableTasks() {
    const db = this.getDb()

    // Get all level 2 tasks (sub-sub-tasks), excluding routines and completed tasks
    const level2Tasks = db.prepare(`
      ...
      LEFT JOIN tasks p ON t.parent_id = p.id
      LEFT JOIN tasks gp ON p.parent_id = gp.id
      WHERE t.level = 2 AND t.type != 'routine' AND (t.is_completed = 0 OR t.is_completed IS NULL)
      ORDER BY
    ...
    // Get level 1 tasks that don't have children, excluding routines and completed tasks
    const level1TasksWithoutChildren = db.prepare(`
      SELECT
        ...
        p.category_id as parentCategoryId,
        NULL as grandparentCategoryId
      FROM tasks t
      LEFT JOIN tasks p ON t.parent_id = p.id
      WHERE t.level = 1 AND t.type != 'routine' AND (t.is_completed = 0 OR t.is_completed IS NULL)
        AND NOT EXISTS (
  ...
  getTasksDueSoon(days: number = 2) {
    ...
    // Filter out completed tasks
    const completedTasksDb = require('./completed-tasks-db').default
    const taskIds = allDueTasks.map((task: any) => task.id)
    const completionStatusMap = completedTasksDb.getTasksCompletionStatus(taskIds)

    const dueTasks = allDueTasks.filter((task: any) => {
      const status = completionStatusMap.get(task.id)
      return !status?.isCompleted
    })

    // Sort by deadline
    dueTasks.sort((a: any, b: any) => a.deadline.localeCompare(b.deadline))
...
export function getAllSchedulableTasks() {
  return tasksDbManager.getAllSchedulableTasks()
}

// Get schedulable routine tasks
export function getSchedulableRoutines() {
  return tasksDbManager.getSchedulableRoutines()
}
